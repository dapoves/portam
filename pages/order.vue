    ...
    <div>
        ...
        <div class="p-6">
            ...
            <div class="my-1 relative h-10 w-full min-w-[200px]">
                ...
                <p v-if="direccion.length > 0 && !validarDireccion(direccion)" class="text-amber-600 text-xs py-1">Asegurate de añadir la calle, número y población</p>

            </div>
            <hr class="border-t-4 mt-7">
            ...
            <div class="flex flex-wrap items-center justify-between cursor-pointer z-1" @click="toggleTarjetas">
                ...
                <p v-if="!tarjetas.length" class="mb-3 font-medium text-sm text-right text-[#434E58] flex items-center"><IconOctagonAlert :size="16" class="mx-1"/> Aún no has añadido ningún método de pago</p>
                <p v-if="tarjetas.length > 0 && !tarjeta_id" class="mb-3 font-medium text-sm w-full text-right text-red-500">Debes seleccionar un método de pago</p>
            </div>
            ...
            <button v-if="orderStore.products.length" @click="pagar"
...
import { validarDireccion } from '~/funciones/validaciones';
